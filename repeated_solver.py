        ...
        if (ret_code == 0):
            ...
            while (1):
                if (mode == "hole_elimination_mode"):
                    ...
                    hole_value_file_string = hole_value_file_string[
                        begin_pos:end_pos + 1]
                    ...
                    for bits in range(2, 10):
                        output_with_counter_example = counter_example_generator(
                            bits, filename, num_fields_in_prog,
                            num_state_groups)
                        print("Bit: %d" % bits)
                        if (output_with_counter_example == ""):
                            ...
                            for i in range(len(state_group)):
                                if (i < len(state_group) - 1):
                                    counter_example_definition += state_group[
                                        i][0] + " = " + str(
                                            int(state_group[i][1]) +
                                            2**bits) + ',\n'
                                else:
                ...
                new_sketch = open("/tmp/" + sketch_name + "_new_sketch.sk",
                                  "w")
                ...
                if (ret_code1 == 0):
                    hole_value_file = open(
                        "/tmp/" + sketch_name + "_result.holes", "w")
                    ...
                    ret_code = sol_verify(
                        "/tmp/" + sketch_name + "_new_sketch.sk",
                        "/tmp/" + sketch_name + "_result.holes")
                    if (ret_code == 0):
